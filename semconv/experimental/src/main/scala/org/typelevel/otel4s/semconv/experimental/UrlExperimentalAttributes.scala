...
object UrlExperimentalAttributes {
  ...
  /** Domain extracted from the `url.full`, such as
    * &quot;opentelemetry.io&quot;.
    *
    * @note
  ...
  /** Unmodified original URL as seen in the event source.
    ...
    *     `https://username:password@www.example.com/`. In such case password
    *     and username SHOULD NOT be redacted and attribute's value SHOULD
    *     remain the same.
    */
  ...
  /** The highest registered url domain, stripped of the subdomain.
    *
    * @note
    *   - This value can be determined precisely with the <a
    *     href="http://publicsuffix.org">public suffix list</a>. For example,
    *     the registered domain for `foo.example.com` is `example.com`. Trying
    *     to approximate this by simply taking the last two labels will not work
    *     well for TLDs such as `co.uk`.
    */
  val UrlRegisteredDomain: AttributeKey[String] = string(
  ...
  /** The subdomain portion of a fully qualified domain name includes all of the
    ...
    *
    * @note
    *   - The subdomain portion of `www.east.mydomain.co.uk` is `east`. If the
