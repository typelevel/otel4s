...
/** This example relies on the OpenTelemetry Java agent. To make it work, add the following settings to your build:
  ...
  *     "io.opentelemetry.instrumentation" % "opentelemetry-instrumentation-annotations" % "2.13.2",
  *     "io.opentelemetry"                 % "opentelemetry-exporter-otlp"               % "1.47.0" % Runtime,
  *     "io.opentelemetry"                 % "opentelemetry-sdk-extension-autoconfigure" % "1.47.0" % Runtime
  *   )
  *   run / fork := true,
  *   javaOptions += "-Dotel.java.global-autoconfigure.enabled=true",
  *   javaOptions += "-Dotel.service.name=pekko-otel4s",
  *   javaAgents += "io.opentelemetry.javaagent" % "opentelemetry-javaagent" % "2.13.2" % Runtime
  * )
  * }}}
  */
