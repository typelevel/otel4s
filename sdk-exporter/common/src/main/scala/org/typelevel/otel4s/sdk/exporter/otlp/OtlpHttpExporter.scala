...
/** Exports spans via HTTP. Support `json` and `protobuf` encoding.
  ...
  */
private final class OtlpHttpExporter[F[_]: Temporal: Console, A] private (
    client: Client[F],
  ...
  def doExport[G[_]: Foldable](records: G[A]): F[Unit] = {
    ...
    client
      ...
      .handleErrorWith { e =>
        Console[F].errorln(
          s"OtlpHttpExporter: cannot export: ${e.getMessage}\n${e.getStackTrace.mkString("\n")}\n"
        )
      }
  }
  ...
  private def logBody(response: Response[F]): F[Unit] =
    for {
      ...
      _ <- Console[F].println(
        s"[OtlpHttpExporter/${config.encoding}] the request failed with [${response.status}]. Body: $body"
      )
    } yield ()
...
private[otlp] object OtlpHttpExporter {
  ...
  def create[F[_]: Async: Network: Compression: Console, A](
      ...
      printer: Printer
  ): Resource[F, OtlpHttpExporter[F, A]] = {
    ...
    } yield new OtlpHttpExporter[F, A](Retry(policy)(gzip(client)), config)
  }
