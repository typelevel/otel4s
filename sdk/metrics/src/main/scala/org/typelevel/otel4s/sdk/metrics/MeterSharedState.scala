...
import org.typelevel.otel4s.sdk.metrics.internal.{CallbackRegistration, InstrumentDescriptor, MetricStorageRegistry, SdkObservableMeasurement}
...
private[metrics] final class MeterSharedState[F[_]: Concurrent](
  ...
  def registerMetricStorage(
    ...
    registries.toVector.flatTraverse { case (reader, registry) =>
      reader.viewRegistry.findViews(descriptor, scope).flatTraverse { registeredView =>
        if (registeredView.view.aggregation == Aggregation.drop) {
          MonadCancelThrow[F].pure(Vector.empty[MetricStorage.Synchronous[F]])
        } else {
          for {
            s <- MetricStorage.synchronous(reader, registeredView, descriptor, exemplarFilter)
  ...
  def registerLongObservableMeasurement(
    ...
    registries.toVector.flatTraverse { case (reader, registry) =>
      reader.viewRegistry.findViews(descriptor, scope).flatTraverse { registeredView =>
        if (registeredView.view.aggregation == Aggregation.drop) {
          MonadCancelThrow[F].pure(Vector.empty[MetricStorage.Asynchronous[F]])
        } else {
          for {
            s <- MetricStorage.asynchronous(reader, registeredView, descriptor)
            _ <- registry.register(s)
          } yield Vector(s)
        }
      }
  ...
  def collectAll(
    ...
    callbacks.get.flatMap { currentCallbacks =>
      mutex.lock.surround {
        currentCallbacks.traverse_ { callback =>
          ...
          for {
            ...
            result <- storages.traverse { storage =>
              storage.collect(resource, scope, startTimestamp, collectTimestamp)
            }
