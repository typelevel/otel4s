...
object NetExperimentalAttributes {
  ...
  */
  @deprecated("Use `server.address` on client spans and `client.address` on server spans", "0.5.0")
  val NetPeerName: AttributeKey[String] = string("net.peer.name")

  /**
  * Deprecated, use `server.port` on client spans and `client.port` on server spans.
  */
  @deprecated("Use `server.port` on client spans and `client.port` on server spans", "0.5.0")
