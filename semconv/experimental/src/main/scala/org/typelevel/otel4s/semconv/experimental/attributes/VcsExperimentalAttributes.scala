...
object VcsExperimentalAttributes {
  ...
  /** The ID of the change (pull request/merge request) if applicable. This is usually a unique (within repository)
    * identifier generated by the VCS system.
    */
  val VcsRepositoryChangeId: AttributeKey[String] =
    AttributeKey("vcs.repository.change.id")

  /** The human readable title of the change (pull request/merge request). This title is often a brief summary of the
    * change and may get merged in to a ref as the commit summary.
    */
  ...
  /** The revision, literally <a href="https://www.merriam-webster.com/dictionary/revision">revised version</a>, The
    ...
    *   which is an integer that is monotonically increasing. In cases where it is identical to the `ref.name`, it
  ...
  /** The <a href="https://en.wikipedia.org/wiki/URL">URL</a> of the repository providing the complete address in order
    * to locate and identify the repository.
    */
