...
/** The context-manipulation interface.
  ...
  */
private[otel4s] sealed trait TraceScope[F[_], Ctx] {
  ...
  /** Creates a scope that uses the given `context`.
    ...
    *   the context to use
    */
  def withContext(context: Ctx): F ~> F

  /** Runs the `f` with the current [[Ctx]].
    *
    * A shortcut for
    * {{{
    ...
    * }}}
    */
  def contextReader[A](f: Ctx => A): F[A]
}

private[otel4s] object TraceScope {

  def fromLocal[F[_], Ctx](
      getSpanContext: Ctx => Option[SpanContext],
      setSpanContext: (Ctx, SpanContext) => Ctx
  )(implicit
      L: Local[F, Ctx],
      C: Contextual[Ctx]
  ): TraceScope[F, Ctx] =
    new TraceScope[F, Ctx] {
      def current: F[Option[SpanContext]] =
        L.reader(getSpanContext)
      ...
      def rootScope: F[F ~> F] =
        L.reader { context =>
          val ctx = getSpanContext(context) match {
      ...
      private def createScope(spanContext: SpanContext): F ~> F =
        new (F ~> F) {
          def apply[A](fa: F[A]): F[A] =
            L.local(fa)(context => setSpanContext(context, spanContext))
        }
