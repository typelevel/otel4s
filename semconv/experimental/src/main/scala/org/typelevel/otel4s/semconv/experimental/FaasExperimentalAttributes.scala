...
object FaasExperimentalAttributes {
  ...
  *
  * @note <li><strong>AWS Lambda:</strong> Use the (full) log stream name.</li>
...
a TracerProvider (see also the `cloud.resource_id` attribute).</li>
  ...
  *
  * @note 
  *  - Depending on the cloud provider and platform, use:<li><strong>AWS Lambda:</strong> The <a href="https://docs.aws.amazon.com/lambda/latest/dg/configuration-versions.html">function version</a>
