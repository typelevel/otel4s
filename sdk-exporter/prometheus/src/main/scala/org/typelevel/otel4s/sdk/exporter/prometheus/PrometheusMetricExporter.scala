...
object PrometheusMetricExporter {
  ...
  sealed trait HttpServerBuilder[F[_]] {
    ...
    /** Sets the flag indicating if metrics are produced without a unit suffix. */
    def withWithoutUnits(withoutUnits: Boolean): HttpServerBuilder[F]

    /** Sets the flag indicating if metrics are produced without a type suffix. */
    def withWithoutTypeSuffixes(withoutTypeSuffixes: Boolean): HttpServerBuilder[F]

    /** Sets the flag indicating if metrics are produced without a scope info metric or scope labels. */
    def withDisableScopeInfo(disableScopeInfo: Boolean): HttpServerBuilder[F]

    /** Sets the flag indicating if metrics are produced without a target info metric. */
    def withDisableTargetInfo(disableTargetInfo: Boolean): HttpServerBuilder[F]
  ...
  def serverBuilder[F[_]: Async: Network: Console]: HttpServerBuilder[F] =
    new HttpServerBuilderImpl[F](
      ...
      port = Defaults.Port,
      defaultAggregationSelector = AggregationSelector.default,
      withoutUnits = Defaults.WithoutUnits,
      withoutTypeSuffixes = Defaults.WithoutTypeSuffixes,
      disableScopeInfo = Defaults.DisableScopeInfo,
      disableTargetInfo = Defaults.DisableTargetInfo
    )
  ...
  private final case class HttpServerBuilderImpl[F[_]: Async: Network: Console](
      host: Host,
      port: Port,
      defaultAggregationSelector: AggregationSelector,
    ...
    def withWithoutUnits(withoutUnits: Boolean): HttpServerBuilder[F] =
      copy(withoutUnits = withoutUnits)

    def withWithoutTypeSuffixes(withoutTypeSuffixes: Boolean): HttpServerBuilder[F] =
      copy(withoutTypeSuffixes = withoutTypeSuffixes)

    def withDisableScopeInfo(disableScopeInfo: Boolean): HttpServerBuilder[F] =
      copy(disableScopeInfo = disableScopeInfo)

    def withDisableTargetInfo(disableTargetInfo: Boolean): HttpServerBuilder[F] =
      copy(disableTargetInfo = disableTargetInfo)
    ...
    def build: Resource[F, MetricExporter.Pull[F]] =
      Resource.eval(Ref.empty[F, Vector[MetricProducer[F]]]).flatMap { ref =>
        ...
        val routes = PrometheusHttpRoutes
          .routes[F](exporter, withoutUnits, withoutTypeSuffixes, disableScopeInfo, disableTargetInfo)

        val metricOptions =
          "{" +
            s"withoutUnits=$withoutUnits, " +
            s"withoutTypeSuffixes=$withoutTypeSuffixes, " +
            s"disableScopeInfo=$disableScopeInfo, " +
            s"disableTargetInfo=$disableTargetInfo}"
