...
object LimitedData {
  ...
  implicit def limitedDataHash[A, S <: immutable.Iterable[A]: Hash]
    ...
    Hash.by(events => (events.limit, events.dropped, events.elements))
  ...
  /** Created [[LimitedData]] with the collection of [[Attribute]] inside.
    ...
    *   The limit for container elements
    */
  def attributes(limit: Int): LimitedData[Attribute[_], Attributes] =
  ...
  /** Created [[LimitedData]] with the vector of [[LinkData]] inside.
    ...
    *   The limit for container elements
    */
  def links(limit: Int): LimitedData[LinkData, Vector[LinkData]] =
  ...
  /** Created [[LimitedData]] with the vector of [[EventData]] inside.
    ...
    *   The limit for container elements
    */
  def events(limit: Int): LimitedData[EventData, Vector[EventData]] =
  ...
  private final case class Impl[A, S <: immutable.Iterable[A]: Semigroup](
    ...
    def appendAll(as: S): LimitedData[A, S] =
      if (elements.size + as.size <= limit) {
        copy(elements = elements |+| as)
      } else {
        val (toAdd, toDrop) = splitAt(as, limit - elements.size)
        copy(elements = elements |+| toAdd, dropped = dropped + toDrop.size)
      }
