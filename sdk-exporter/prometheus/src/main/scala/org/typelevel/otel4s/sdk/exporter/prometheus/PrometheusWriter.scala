...
object PrometheusWriter {
  ...
  def text[F[_]: MonadThrow: Console](
    ...
    new TextWriter[F](withoutUnits, withoutTypeSuffixes, disableScopeInfo, disableTargetInfo)
  ...
  /** Writes metrics using the Prometheus text format.
    ...
    */
  private final class TextWriter[F[_]: Console](
      ...
      F.fromEither {
        ...
        aggregate.groups.get(prometheusName) match {
          case Some(group) if group.prometheusType == prometheusType =>
            ...
            aggregate
              .tryAddScope(metric.instrumentationScope, disableScopeInfo)
              .tryUpdateResource(metric.resource, disableTargetInfo)
    ...
    private def serializeScopes(scopes: NonEmptyVector[InstrumentationScope]): Either[Throwable, String] = {
      ...
      scopes
        .traverse { scope =>
          scope.attributes.toList
            .traverse(attr => convertLabelName(attr.key.name).map(_ -> attr.value.toString))
            .map(ListMap.from(_))
    ...
    private def serializeSums(metricGroup: MetricGroup): Either[Throwable, String] = {
      val typeSuffix = if (withoutTypeSuffixes) "" else "_total"
