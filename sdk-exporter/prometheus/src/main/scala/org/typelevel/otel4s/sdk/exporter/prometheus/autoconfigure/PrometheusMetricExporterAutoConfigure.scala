  ...
  protected def fromConfig(config: Config): Resource[F, MetricExporter[F]] =
    for {
      ...
      exporter <- PrometheusMetricExporter
        ...
        .withWithoutUnits(withoutUnits)
        .withWithoutTypeSuffixes(withoutTypeSuffixes)
        .withDisableScopeInfo(disableScopeInfo)
        .withDisableTargetInfo(disableTargetInfo)
        .build
    } yield exporter
