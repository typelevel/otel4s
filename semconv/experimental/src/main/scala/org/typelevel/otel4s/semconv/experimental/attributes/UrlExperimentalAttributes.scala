...
object UrlExperimentalAttributes {
  ...
  /** Domain extracted from the `url.full`, such as "opentelemetry.io". <p>
    * @note
    *   <p> In some cases a URL may refer to an IP and/or port directly, without
    *   a domain name. In this case, the IP address would go to the domain
    *   field. If the URL contains a <a
    *   href="https://www.rfc-editor.org/rfc/rfc2732#section-2">literal IPv6
    *   address</a> enclosed by `[` and `]`, the `[` and `]` characters should
    *   also be captured in the domain field.
    */
  ...
  /** The file extension extracted from the `url.full`, excluding the leading
    * dot. <p>
    * @note
    *   <p> The file extension is only set if it exists, as not every url has a
    *   file extension. When the file name has multiple extensions
  ...
  /** Absolute URL describing a network resource according to <a
    ...
    * @note
    *   <p> For network calls, URL usually has
    *   `scheme://host[:port][path][?query][#fragment]` format, where the
    *   fragment is not transmitted over HTTP, but if it is known, it SHOULD be
    *   included nevertheless. `url.full` MUST NOT contain credentials passed
    *   via URL in form of `https://username:password@www.example.com/`. In such
    *   case username and password SHOULD be redacted and attribute's value
    *   SHOULD be `https://REDACTED:REDACTED@www.example.com/`. `url.full`
    *   SHOULD capture the absolute URL when it is available (or can be
  ...
  /** Unmodified original URL as seen in the event source. <p>
    * @note
    *   <p> In network monitoring, the observed URL may be a full URL, whereas
  ...
  /** The <a href="https://www.rfc-editor.org/rfc/rfc3986#section-3.3">URI
    * path</a> component <p>
    * @note
    *   <p> Sensitive content provided in `url.path` SHOULD be scrubbed when
  ...
  /** The <a href="https://www.rfc-editor.org/rfc/rfc3986#section-3.4">URI
    * query</a> component <p>
    * @note
    *   <p> Sensitive content provided in `url.query` SHOULD be scrubbed when
    *   instrumentations can identify it.
    */
  @deprecated(
  ...
  /** The highest registered url domain, stripped of the subdomain. <p>
    * @note
    *   <p> This value can be determined precisely with the <a
    *   href="http://publicsuffix.org">public suffix list</a>. For example, the
    *   registered domain for `foo.example.com` is `example.com`. Trying to
  ...
  /** The subdomain portion of a fully qualified domain name includes all of the
    ...
    * determined, subdomain contains all of the names below the registered
    * domain. <p>
    * @note
    *   <p> The subdomain portion of `www.east.mydomain.co.uk` is `east`. If the
