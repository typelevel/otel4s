...
object BrowserExperimentalAttributes {
  ...
  /** Array of brand name and version separated by a space <p>
    ...
    *   API</a> (`navigator.userAgentData.brands`).
    */
  ...
  /** Preferred language of the user using the browser <p>
    * @note
    *   <p> This value is intended to be taken from the Navigator API
    *   `navigator.language`.
    */
  ...
  /** A boolean that is true if the browser is running on a mobile device <p>
    ...
    *   <p> This value is intended to be taken from the <a
    *   href="https://wicg.github.io/ua-client-hints/#interface">UA client hints
    *   API</a> (`navigator.userAgentData.mobile`). If unavailable, this
  ...
  /** The platform on which the browser is running <p>
    ...
    *   SHOULD be left unset in order for the values to be consistent. The list
    *   of possible values is defined in the <a
    *   href="https://wicg.github.io/ua-client-hints/#sec-ch-ua-platform">W3C
    *   User-Agent Client Hints specification</a>. Note that some (but not all)
    *   of these values can overlap with values in the <a
