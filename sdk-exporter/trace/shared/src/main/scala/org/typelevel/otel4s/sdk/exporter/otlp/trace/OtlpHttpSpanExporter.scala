...
private final class OtlpHttpSpanExporter[F[_]: Temporal: Console] private (
  ...
  import OtlpHttpSpanExporter.Encoding
    ...
    config.encoding match {
      case Encoding.Json =>
        ...
        jsonEncoderWithPrinter(Printer.noSpaces)
          .contramap[List[SpanData]](spans => ProtoEncoder.toJson(spans))
...
object OtlpHttpSpanExporter {
  ...
  private final case class BuilderImpl[
    ...
    def build: Resource[F, SpanExporter[F]] = {
      ...
      } yield new OtlpHttpSpanExporter[F](Retry(policy)(gzip(client)), config)
    }
  }
