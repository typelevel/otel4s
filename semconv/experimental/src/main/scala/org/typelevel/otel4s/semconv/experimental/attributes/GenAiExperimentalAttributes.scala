...
object GenAiExperimentalAttributes {
  ...
  /** The name of the operation being performed. <p>
    * @note
    *   <p> If one of the predefined values applies, but specific system uses a
  ...
  /** The Generative AI product as identified by the client or server
    * instrumentation. <p>
    * @note
    *   <p> The `gen_ai.system` describes a family of GenAI models with specific
    *   model identified by `gen_ai.request.model` and `gen_ai.response.model`
    *   attributes. <p> The actual GenAI product may differ from the one
    *   identified by the client. For example, when using OpenAI client
    *   libraries to communicate with Mistral, the `gen_ai.system` is set to
    *   `openai` based on the instrumentation's best knowledge. <p> For custom
    *   model, a custom friendly name SHOULD be used. If none of these options
  ...
  object GenAiOperationNameValue {
    ...
    /** Chat completion operation such as <a
      * href="https://platform.openai.com/docs/api-reference/chat">OpenAI Chat
      * API</a>
      */
    case object Chat extends GenAiOperationNameValue("chat")

    /** Text completions operation such as <a
      * href="https://platform.openai.com/docs/api-reference/completions">OpenAI
