...
object OtelExperimentalMetrics {
  ...
  /** The number of created spans for which the end operation was called
    *
    * @note
    *   <p> For spans with `recording=true`: Implementations MUST record both `otel.sdk.span.live.count` and
    *   `otel.sdk.span.ended.count`. For spans with `recording=false`: If implementations decide to record this metric,
    *   they MUST also record `otel.sdk.span.live.count`.
    */
  ...
  /** The number of created spans for which the end operation has not been called yet
    ...
    *   <p> For spans with `recording=true`: Implementations MUST record both `otel.sdk.span.live.count` and
    *   `otel.sdk.span.ended.count`. For spans with `recording=false`: If implementations decide to record this metric,
    *   they MUST also record `otel.sdk.span.ended.count`.
    */
