...
abstract class TraceScopeSuite[Ctx, K[X] <: Key[X]](implicit
  ...
  test("rootScope - use Context.root when there is no SpanContext") {
    ...
    for {
      ...
      key <- Key.Provider[IO, K].uniqueKey[String]("some-key")
  ...
  test("withContext - use the given context") {
    PropF.forAllF { (value: String) =>
      for {
        ...
        _ <- assertIO(scope.contextReader(identity), C.root)

        _ <- scope.withContext(C.root.updated(key, value)) {
          assertIO(
  ...
  protected def createTraceScope: IO[TraceScope[IO, Ctx]]
