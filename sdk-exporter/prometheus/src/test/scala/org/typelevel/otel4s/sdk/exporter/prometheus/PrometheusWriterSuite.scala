...
class PrometheusWriterSuite extends CatsEffectSuite {
  ...
  test("write MetricData with monotonic MetricPoints.Sum") {
    val expected =
      s"""# HELP foo_milliseconds_total a simple counter
         ...
         |target_info{telemetry_sdk_name="otel4s",telemetry_sdk_language="scala",telemetry_sdk_version="${BuildInfo.version}",service_name="unknown_service:scala"} 1
         |""".stripMargin
  ...
  test("write MetricData with monotonic MetricPoints.Sum when type suffixes are disabled") {
    ...
    writeAndCompare(metrics, expected, withoutTypeSuffixes = true)
  }
  ...
  test("merge or overwrite label values after keys conversion") {
    ...
    writeAndCompare(metrics, expected, withoutUnits = true)
  }
  ...
  test("write result without target_info when it is disabled") {
    ...
    writeAndCompare(metrics, expected, disableTargetInfo = true)
  }
  ...
  test("write result without scope_info when it is disabled") {
    ...
    writeAndCompare(metrics, expected, disableScopeInfo = true)
  }
  ...
  test("write result without scope_info and target_info when they are disabled") {
    ...
    writeAndCompare(metrics, expected, disableScopeInfo = true, disableTargetInfo = true)
  }
  ...
  test("drop gauge conflicting with counter with the same name") {
    ...
    InMemoryConsole.create[IO].flatMap { implicit C: InMemoryConsole[IO] =>
      for {
        _ <- writeAndCompare(metrics, expectedText, withoutUnits = true)
  ...
  private def writeAndCompare(
    ...
    PrometheusWriter
      .text[IO](withoutUnits, withoutTypeSuffixes, disableScopeInfo, disableTargetInfo)
      .write(metrics)
      .compile
      .to(Array)
