...
object RpcExperimentalMetrics {
  ...
  object ClientCallDuration extends MetricSpec.Unsealed {
    ...
    val description: String = "Measures the duration of an outgoing Remote Procedure Call (RPC)."
    val unit: String = "s"
    ...
    object AttributeSpecs {
      ...
      /** A string identifying a group of RPC server instances request is sent to.
        *
        * @note
        *   <p> May contain a DNS name, an endpoint and path in the service registry, local socket name or an IP
        *   address. Semantic conventions for individual RPC systems SHOULD document how to populate this attribute.
        *   When address is an IP address, instrumentations SHOULD NOT do a reverse DNS lookup to obtain a DNS name and
        *   SHOULD set `server.address` to the provided IP address.
        */
      ...
      val serverPort: AttributeSpec[Long] =
        AttributeSpec(
          ...
          Requirement.conditionallyRequired("if applicable and if `server.address` is set."),
          Stability.stable
        )
  ...
  object ServerCallDuration extends MetricSpec.Unsealed {
    ...
    val description: String = "Measures the duration of an incoming Remote Procedure Call (RPC)."
    val unit: String = "s"
    val stability: Stability = Stability.releaseCandidate
    val attributeSpecs: List[AttributeSpec[_]] = AttributeSpecs.specs
