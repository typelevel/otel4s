...
class SpansProtoEncoderSuite extends ScalaCheckSuite {
  ...
  test("encode EventData (noSpaces)") {
    ...
    assertEquals(
      ProtoEncoder
        .toJson(EventData("name", 1.nanos, LimitedData.attributes(100)))
        .noSpaces,
      ...
      ProtoEncoder
        .toJson(
          EventData(
            "name",
            1.nanos,
            LimitedData.attributes(attrs.size).appendAll(attrs)
          )
        )
        .noSpaces,
  ...
  test("encode LinkData (noSpaces)") {
    ...
    assertEquals(
      ProtoEncoder.toJson(LinkData(ctx, LimitedData.attributes(100))).noSpaces,
    ...
    assertEquals(
      ProtoEncoder
        .toJson(
          LinkData(ctx, LimitedData.attributes(attrs.size).appendAll(attrs))
        )
        .noSpaces,
    ...
    assertEquals(
      ProtoEncoder.toJson(LinkData(ctx2, LimitedData.attributes(100))).noSpaces,
