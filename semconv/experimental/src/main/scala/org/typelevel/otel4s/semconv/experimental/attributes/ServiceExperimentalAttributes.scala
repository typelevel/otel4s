...
object ServiceExperimentalAttributes {
  ...
  /** The string ID of the service instance. <p>
    * @note
    *   <p> MUST be unique for each instance of the same
    ...
    *   scaled service). <p> Implementations, such as SDKs, are recommended to
    *   generate a random Version 1 or Version 4 <a
    ...
    *   stability is desirable. In that case, the ID SHOULD be used as source of
    *   a UUID Version 5 and SHOULD use the following UUID as the namespace:
    *   `4d63009a-8d0f-11ee-aad7-4c796ed8e320`. <p> UUIDs are typically
    *   recommended, as only an opaque value for the purposes of identifying a
    *   service instance is needed. Similar to what can be seen in the man page
    *   for the <a
    ...
    *   treated as confidential, being the user's choice to expose it or not via
    *   another resource attribute. <p> For applications running behind an
    *   application server (like unicorn), we do not recommend using one
    *   identifier for all processes participating in the application. Instead,
    *   it's recommended each division (e.g. a worker thread in unicorn) to have
    *   its own instance.id. <p> It's not recommended for a Collector to set
    *   `service.instance.id` if it can't unambiguously determine the service
    *   instance that is generating that telemetry. For instance, creating an
    *   UUID based on `pod.name` will likely be wrong, as the Collector might
    *   not know from which container within that pod the telemetry originated.
    *   However, Collectors can set the `service.instance.id` if they can
    *   unambiguously determine the service instance for that telemetry. This is
    *   typically the case for scraping receivers, as they know the target
    *   address and port.
    */
  ...
  /** A namespace for `service.name`. <p>
    * @note
    *   <p> A string value having a meaning that helps to distinguish a group of
