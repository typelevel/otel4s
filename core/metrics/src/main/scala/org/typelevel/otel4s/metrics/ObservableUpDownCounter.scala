...
object ObservableUpDownCounter {
  ...
  object Builder {
    private class MappedK[F[_]: MonadCancelThrow, G[_]: MonadCancelThrow, A](inner: Builder[F, A])(implicit
      ...
      override def createWithCallback(
          cb: ObservableMeasurement[G, A] => G[Unit]
      ): Resource[G, ObservableUpDownCounter] = {
      ...
      override def create(measurements: G[Iterable[Measurement[A]]]): Resource[G, ObservableUpDownCounter] =
